...
<link rel="stylesheet" type="text/css" href="stylesVentas.css">
    <div class="container">
        <div class="EditarClientesTitulo"><i class="fa-solid fa-sack-dollar"></i> Ventas</div>
        <div class="row">
            <div class="col-md-4">
                ...
                <div class="tarjeta2">
                    <form action="php/crud/saveTaskVentaNueva.php" method="POST">
                        <div class="form-group">
                            <input type="text" placeholder="Codigo cliente" name="numeroCliente">
                        </div>
                        <div class="form-group2">
                            <input type="number" placeholder="x20" name="x20">
                        </div>
                        <div class="form-group2">
                            <input type="number" placeholder="x12" name="x12">
                        </div>
                        <div class="form-group2">
                            <input type="number" placeholder="x5" name="x5">
                        </div>
                        <div class="form-group2">
                            <input type="number" placeholder="Sifon de soda" name="sifonSoda">
                        </div>
                        <div class="form-group2">
                            <input type="number" placeholder="Alquier frio-Calor" name="alqfriocalor">
                        </div>
                        <div class="form-group2">
                            <input type="number" placeholder="Dispenser" name="dispenser">
                        </div>
                        <div class="form-group2">
                            <input type="number" placeholder="Canilla" name="canilla">
                        </div>
                        <div class="form-group2">
                            <input type="date" name="fecha" value="<?php echo $fecha_year; echo '-'; echo $fecha_mes; echo '-'; echo $fecha_dia?>">
                        </div>
                        <div class="form-group3">
                            <input type="number" placeholder="Pagó" name="pago">
                        </div>
                            <input type="submit" class="boton-submit-cliente" name="saveTask" value="Guardar venta">
                    </form>
                </div>
                ...
                <div class="tarjeta3"> <p><?php $query=mysqli_query($conexion,"SELECT SUM(montoPagado) AS totalPagado FROM ventas WHERE fecha='$fecha_year-$fecha_mes-$fecha_dia'");
                $result=mysqli_fetch_array($query);
                    echo "Cobrado: $".$result['totalPagado'];
                ?> </p>
                <p><?php $query=mysqli_query($conexion,"SELECT SUM(bidones20) AS totalx20 FROM ventas WHERE fecha='$fecha_year-$fecha_mes-$fecha_dia'");
                $result=mysqli_fetch_array($query);
                    echo "x20: ".$result['totalx20'];
                ?> </p>
                <p><?php $query=mysqli_query($conexion,"SELECT SUM(bidones12) AS totalx12 FROM ventas WHERE fecha='$fecha_year-$fecha_mes-$fecha_dia'");
                $result=mysqli_fetch_array($query);
                    echo "x12: ".$result['totalx12'];
                ?> </p>
                <p><?php $query=mysqli_query($conexion,"SELECT SUM(bidones5) AS totalx5 FROM ventas WHERE fecha='$fecha_year-$fecha_mes-$fecha_dia'");
                $result=mysqli_fetch_array($query);
                    echo "x5: ".$result['totalx5'];
                ?> </p>
                <p><?php $query=mysqli_query($conexion,"SELECT SUM(sifones) AS sifones FROM ventas WHERE fecha='$fecha_year-$fecha_mes-$fecha_dia'");
                $result=mysqli_fetch_array($query);
                    echo "Sifones: ".$result['sifones'];
                ?> </p>
                <p><?php $query=mysqli_query($conexion,"SELECT SUM(alqfriocalo) AS alqfriocalo FROM ventas WHERE fecha='$fecha_year-$fecha_mes-$fecha_dia'");
                $result=mysqli_fetch_array($query);
                    echo "Frio/calor: ".$result['alqfriocalo'];
                ?> </p>
                <p><?php $query=mysqli_query($conexion,"SELECT SUM(dispenser) AS dispenser FROM ventas WHERE fecha='$fecha_year-$fecha_mes-$fecha_dia'");
                $result=mysqli_fetch_array($query);
                    echo "Dispenser: ".$result['dispenser'];
                ?> </p>
                <p><?php $query=mysqli_query($conexion,"SELECT SUM(canilla) AS canilla FROM ventas WHERE fecha='$fecha_year-$fecha_mes-$fecha_dia'");
                $result=mysqli_fetch_array($query);
                ...
                while($roww = mysqli_fetch_array($resultado_tareas2)){ 
                    ...
                    $query=mysqli_query($conexion,"SELECT SUM(montoPagado) AS totalPagados FROM ventas WHERE fecha='$fecha_year-$fecha_mes-$fecha_dia' AND encargado='$usuario'");
                    $result=mysqli_fetch_array($query);
                    ?>
                    ...
                    <table>
                        ...
                        <tbody>
                            <?php 
                                ...
                                $sql_register = mysqli_query($conexion, "SELECT COUNT(*) as total_ventas FROM `ventas` WHERE fecha='$fecha_year-$fecha_mes-$fecha_dia'");
                                ...
                                $query = "SELECT * FROM ventas WHERE fecha='$fecha_year-$fecha_mes-$fecha_dia' ORDER BY id DESC LIMIT $desde,$porPagina";
