<?php
    ...
    if(isset($_POST['guardarReparto'])){
        ...
        $sql = "UPDATE clientes SET saldoAnteriorCliente = $nuevoSaldoTotal, paraManana = '$paraManana' WHERE nombreCliente='$nombre' AND direccionCliente='$direccionCliente'" ;
        $sqlPassed = "UPDATE clientes SET pasado = $pasado WHERE id = $id";
        $sql = mysqli_query($conexion, $sql);
        $resultado = mysqli_query($conexion, $query);
        $sqlPassed = mysqli_query($conexion, $sqlPassed);
        if(!$resultado) {
            die("Query failed");
        }
        ...
        if($pagina>1){
            header("Location: ../reparto.php?pagina=".$pagina);
        }else{
