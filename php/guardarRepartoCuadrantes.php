<?php
    ...
    if(isset($_POST['guardarReparto'])){
        ...
        $sql = "UPDATE clientes SET saldoAnteriorCliente = $nuevoSaldoTotal, paraManana = '$paraManana', pasado = $pasado WHERE numeroCliente = $idCliente";
        $sql = mysqli_query($conexion, $sql);
        $resultado = mysqli_query($conexion, $query);
        if(!$resultado) {
            die("Query failed");
        }
        $sql = "UPDATE clientes SET pasado = $paraManana WHERE numeroCliente = $idCliente";
        $sql = mysqli_query($conexion, $sql);

        if($cuadrante=="1"){
            $cuadranteVolver = "Cuadrante1";
        }else if($cuadrante=="2"){
            $cuadranteVolver = "Cuadrante2";
        }else if($cuadrante=="3"){
            $cuadranteVolver = "Cuadrante3";
        }else if($cuadrante=="4"){
            $cuadranteVolver = "Cuadrante4";
        }
        
        if($pagina>1){
            header("Location: ../reparto".$cuadranteVolver.".php?pagina=".$pagina);
        }else{
            header("Location: ../reparto".$cuadranteVolver.".php");
        }
    }
